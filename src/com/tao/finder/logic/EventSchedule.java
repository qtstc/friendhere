...
/**
 * A data structure used to store the schedule of events in the phone storage.
 * 
 * In the current implementation, data is converted to a string and then stored
...
public class EventSchedule {
	...
	// Delimiter used to separate different events in the string representation
	...
	public void save(Context c) {
		...
		sp.edit().putString(EVENT_SCHEDULE_SHAREDPREFERENCES_KEY, toString())
				.commit();
	}

	/**
	 * Load existing instance from the SharedPreferences or create a new one if
	 * nothing exists.
	 * 
	 * @param c
	...
	public static EventSchedule getInstance(Context c) {
		...
		while (st.hasMoreTokens()) {
			...
			String name = EventTime.parseEventTimeFromString(next, et);// Parse
																		// instance
																		// from
																		// string
			if (et.getEnding() > currentTime)// We only care about the events
												// that haven't ended.
	 ...
	 * @param et
	 *            the EventTime instance associated with this event
	 */
	...
	/**
	 * Remove an event from the schedule. It's safe to call this method even
	...
	/**
	 * Determine whether the updater should be started based on the event
	 * schedule. This method is to be called by an LocationUpdateScheduler
	 * instance
	 * 
	 * @param currentEventID
	 *            the objectId of the event that invoked the
	 *            LocationUpdateScheduler instance
	 * @param isStarting
	 *            true if the LocationUpdateScheduler is invoked at the
	...
	/**
	 * A data structure used to store the start and ending time of individual
	 * events. It is used in the EventSchedule.
	 * 
	 * @author Tao Qian(taoqian_2015@depauw.edu)
	 * 
	 */
